    ...
    def fit(self, train_iter, optimizer, lr_scheduler,
        ...
        for i_epoch in range(epoch_start, epoch_end):
            ...
            for i_batch, (data, target) in enumerate(train_iter):
                ...
                # [backward]
                optimizer.zero_grad()
                logging.info(f'Train:{i_epoch}-{i_batch}-{losses[0]}')
                ...
                if i_batch % 20 == 0:
                    _, (input, target, _, _) = eval_itr.__next__()
                    output = self.net(input.cuda())
                    acc = find_idx(output, Hash_center, target, self.criterion)
                    logging.info(f'Test:{i_epoch}-{i_batch}-{acc}')
            ...
            if (eval_iter is not None) \
                ...
                for i_batch, (data, target, src_file) in enumerate(eval_iter):
                    self.callback_kwargs['batch'] = i_batch
...
def find_idx(output, hash_center, target, criterion):
    output = output.data.cpu().float()
    real_count = len(output)
    for j in range(len(output)):
        out = output[j]
        hc = hash_center[target[j]]
        min_loss = criterion(0.5 * (out + 1), 0.5 * (hc + 1))
